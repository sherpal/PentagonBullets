...
import graphics.pixitexturemakers.{
  BarrierTextureMaker,
  GunTurretTextureMaker,
  LaserLauncherTextureMaker,
...
object EntityDrawer {
  ...
  List(
    mistStage,
    laserLauncherAnimationStage,
    healingZoneStage,
    damageZoneStage,
    bulletAmplifierStage,
    barrierStage,
    healingUnitStage,
    abilityGiverStage,
    playerStage,
  ...
  def newDisk(center: Complex, radius: Int, r: Double, g: Double, b: Double, alpha: Double = 1.0): Sprite = {
    val (x, y) = (2 * radius, 2 * radius) //canvas2d.changeCoordinates(center)

    val disk = new Sprite(
      Engine.graphics.webGLRenderer.generateTexture(
        new PIXIGraphics()
  ...
  def newPlayer(
      ...
      plrColor: (Double, Double, Double),
      teamColor: (Double, Double, Double)
  ): Sprite = {
  ...
  def drawBarriers(state: GameState, colors: Map[Long, (Double, Double, Double)]): Unit = {
    barrierSprites.toMap
      .filterNot(e => state.barriers.isDefinedAt(e._1))
      .foreach({
        case (id, sprite) =>
          barrierSprites -= id
  ...
  def drawBulletAmplifiers(state: GameState, colors: Map[Long, (Double, Double, Double)]): Unit = {
    bulletAmplifierSprites.toMap
      .filterNot(e => state.bulletAmplifiers.isDefinedAt(e._1))
      .foreach({
        case (id, sprite) =>
    ...
    state.bulletAmplifiers.foreach({
      case (id, bulletAmplifier) =>
  ...
  def drawHealingZones(state: GameState, colors: Map[Long, (Double, Double, Double)]): Unit = {
    healingZoneSprites.toMap.foreach({
      case (id, sprite) =>
  ...
  def drawPlayersPIXI(
    ...
    players
      .filterNot(elem => playersAlive.isDefinedAt(elem._1))
      .foreach(elem => {
      ...
      case (id, player) =>
        val (polygon, circle) = players.get(id) match {
          ...
          case None =>
            val elem = (
              newPlayer(
                player.shape.vertices.map(z => complex.Complex(z.re, z.im)),
                colors(player.id),
  ...
  def drawTeamFlags(state: GameState, colors: Map[Int, (Double, Double, Double)]): Unit = {
    teamFlagSprites
      .filterNot(elem => state.flags.isDefinedAt(elem._1))
      .foreach(elem => {
  ...
  def drawGunTurrets(state: GameState, colors: Map[Long, (Double, Double, Double)]): Unit = {
    gunTurretSprites
      .filterNot(elem => state.gunTurrets.isDefinedAt(elem._1))
      .foreach(elem => {
        gunTurretStage.removeChild(elem._2)
        gunTurretSprites -= elem._1
      })

    state.gunTurrets.foreach({
      case (turretId, turret) =>
  ...
  def drawLaserLauncherSprites(state: GameState, colors: Map[Long, (Double, Double, Double)]): Unit = {
    laserLauncherSprites
      .filterNot(elem => state.laserLaunchers.isDefinedAt(elem._1))
      .foreach(elem => {
        laserLauncherStage.removeChild(elem._2)
        laserLauncherSprites -= elem._1
      })

    state.laserLaunchers.foreach({
      case (laserLauncherId, laserLauncher) =>
...
//            elem.position.set(x - HealUnit.radius, y - HealUnit.radius)
  ...
  def drawObstaclesPIXI(obs: Map[Long, Obstacle]): Unit = {
    obstacles
      .filterNot(elem => obs.isDefinedAt(elem._1))
      .foreach(elem => {
    ...
    obs.foreach({
      case (id, obstacle) =>
        camera.viewportManager(
          obstacles.get(id) match {
            case Some(elem) =>
              elem
            case None =>
              val elem = newPolygon(
                obstacle.shape.asInstanceOf[Polygon].vertices.map(z => complex.Complex(z.re, z.im)),
                1,
                1,
                1
              )
  ...
  /**
    ...
    */
  def drawState(
      cameraPos: custommath.Complex,
      state: GameState,
      time: Long,
      colors: Map[Long, (Double, Double, Double)],
      teamColors: Map[Int, (Double, Double, Double)],
      bulletColors: Map[Long, (Double, Double, Double)]
  ): Unit = {
