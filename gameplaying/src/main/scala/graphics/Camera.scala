...
import pixigraphics.{PIXIContainer, Sprite}


class Camera(canvas: html.Canvas) {
...
//    math.max(left, boundingBox.left + center.re) < math.min(right, boundingBox.right + center.re) &&
//    boundingBox.intersect(cameraBoundingBox, center)
   ...
   * Gives the visibility, the position and the scales of the sprite.
   *
   * @param container         [[PIXIContainer]] to give the spec to.
   * @param worldPos          The position in world coordinates at which the sprite must be anchored.
   ...
   */
  def viewportManager(container: PIXIContainer, worldPos: Complex,
                      boundingBoxCenter: Complex, boundingBox: BoundingBox): Unit = {
  ...
  def viewportManagerSized(container: PIXIContainer, worldPos: Complex, worldWidth: Double, worldHeight: Double,
                           boundingBoxCenter: Complex, boundingBox: BoundingBox): Unit = {
    if (inView(boundingBoxCenter, boundingBox)) {
      if (!container.visible) {
        container.visible = true
   ...
   */
  def viewportManager(container: PIXIContainer, worldPos: Complex,
                      boundingBoxCenter: Complex, boundingBox: BoundingBox,
                      sx: Double, sy: Double): Unit = {
