...
final case class UseAbilityAction(
  ...
  def applyDefault(gameState: GameState): GameState = {
    ...
    gameState.withPlayer(player.id, time, new Player(
      player.id, player.team, time, player.name, player.xPos, player.yPos, player.direction,
      player.speed, player.moving, player.rotation, player.shape,
      player.lifeTotal, player.allowedAbilities, player.relevantUsedAbilities + (useId -> ability)
    ))
  }
