...
final case class PlayerHitByBullet(
  ...
  def applyDefault(gameState: GameState): GameState = gameState.removeBullet(bulletId, time).withPlayer(
    playerId, time, Player.bulletHitPlayer(
      gameState.players(playerId), damage, time
    )
  )
