...
object AbilityGiver {
  ...
  def playerTakeAbilityGiver(player: Player, time: Long, abilityId: Int): Player = new Player(
    player.id, player.team, time, player.name, player.xPos, player.yPos, player.direction, player.speed,
    player.moving, player.rotation, player.shape, player.lifeTotal, player.allowedAbilities :+ abilityId,
