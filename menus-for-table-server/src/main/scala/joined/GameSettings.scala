...
object GameSettings {
  ...
  launchGameButton.addEventListener[dom.MouseEvent](
    "click",
    (_: dom.MouseEvent) => {
  ...
  dom.document
    .getElementById("quitStandardTable")
  ...
  def receivedTableInfoMessage(message: TableServerMessages): Unit = message match {
    case StandardTableAllInfoMessage(tableName, tableEnterPassword, mode, playersInfo)
        if _joinedTable.isDefined &&
      ...
      _playersInfo
        .map(info => (info, PlayerLine.playerLine(info.playerName)))
        .map({
          case (info, None) =>
  ...
  def receivedCreateServerMessage(msg: CreateServer): Unit = {
    ...
    DataStorage.storeGlobalValue(
      "gameData",
      OneTimeServerGameData(
        gameData.get.gameName,
        gameData.get.gameMode,
        gameData.get.reservationId,
