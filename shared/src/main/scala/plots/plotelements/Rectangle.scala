...
class Rectangle(x: Double, y: Double, width: Double, height: Double, color: Vec4) extends Region {
  ...
  private val vertices: Vector[Complex] = Vector(
    Complex(x, y), Complex(x, y - height), Complex(x + width, y - height), Complex(x + width, y)
  )
  ...
  override def closestPointToCanvasCoords(z: Complex, plot: Plot): Complex =
    edges
      .map({ case (z1, z2) => (Complex(plot.plotToCanvasCoordinates(z1.re, z1.im)),
        Complex(plot.plotToCanvasCoordinates(z2.re, z2.im)))})
      .map({ case (z1, z2) => PlotElement.closestPointToInterval(z, z1, z2) }).minBy(w => (z - w).modulus2)
