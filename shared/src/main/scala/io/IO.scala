...
object IO {
  ...
  def readFile(directory: String): Future[Buffer] = {
    ...
    FileSystem.readFile(Path.join(baseDirectory, directory), (err: js.Error, data: js.UndefOr[Buffer]) => {
   ...
   */
  def writeFileContent(directory: String, fileContent: FileContent, errorCallback: (Throwable) => Any = (_) => {})
  : Unit = {
    try {
  ...
  def open(directory: String, flags: String = "r"): Future[Int] = {
    val p = Promise[Int]()
    FileSystem.open(Path.join(baseDirectory, directory), flags, (err: js.Error, fd: js.UndefOr[Int]) => {
      if (err != null && !js.isUndefined(err)) {
        p.failure(errToThrowable(err))
      } else {
        p.success(fd.get)
